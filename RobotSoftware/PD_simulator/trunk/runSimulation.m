function outdata = runSimulation(ptarget,rtarget,insertdist,blockCorners,debuglevel,time1,TotalTime,SABiRx,Rneedle,sys)

nBlocks=size(blockCorners,3);
...
gelParams1insert = [.015 .002 .0023]; %parameters for inserting into gel type 1
gelParams1retract = [-.01 -.002 -.0023];%parameters for retracting from gel type 1
gelParams2insert = [.03 .004 .0046]; %parameters for inserting into gel type 2
gelParams2retract =  [-.02 -.004 -.0046]; %parameters for retracting from gel type 2
